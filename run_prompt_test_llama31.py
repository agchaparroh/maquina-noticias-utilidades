...
def _clean_json_string(raw_string: Optional[str]) -> Optional[str]:
    ...
    fixed_string = re.sub(r"'\s*([^'\s]+)\s*'\s*:", r'"":', fixed_string)

    # 2c. Corregir problemas específicos de comillas en citas
    # Problema común: ""texto"" -> "texto"
    fixed_string = re.sub(r'"cita":\s*""([^"]*?)""', r'"cita": ""', fixed_string)
    ...
    fixed_string = re.sub(r'None', 'null', fixed_string) # Python None -> JSON null
    fixed_string = re.sub(r'True', 'true', fixed_string) # Python True -> JSON true
    fixed_string = re.sub(r'False', 'false', fixed_string) # Python False -> JSON false
